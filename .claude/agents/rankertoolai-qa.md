---
name: rankertoolai-qa
description: Validates a RankerToolAI page (review/comparison/alternatives/best-of) against a strict HTML/SEO/affiliate checklist before it ships. Use before any deploy, or to re-check a page after a fix. Returns PASS/FAIL with a detailed report — never modifies files itself.
...
[ ] No affiliate links pointing to /go/ paths that don't have a real go/[slug]/index.html page (there is no Nginx — /go/ is a static file, check the file exists)
