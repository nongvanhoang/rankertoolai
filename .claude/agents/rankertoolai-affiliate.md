---
name: rankertoolai-affiliate
description: Manages the affiliate program database, supplies /go/[slug]/ tracking URLs to writer agents, and verifies affiliate links are active before a page ships. Use before any writer agent needs a tracking URL, or to audit link health.
...
RankerToolAI has no Nginx and no server-side redirect config — `/go/[slug]/` is a real static file at `go/[slug]/index.html` (same pattern as RankerNest's `templates/go_template.html`), built by `generate_go_pages.py`, that meta-refreshes/JS-redirects to the tracked affiliate URL. There is nothing to "insert into a config file."

When a new affiliate program is added:

```
1. Confirm go/[slug]/index.html doesn't already exist
2. Add a new entry to the TOOLS list at the top of generate_go_pages.py
   (slug, name, dest URL with tracking params, title, heading, badge copy,
   colors, benefits, footer_links — copy an existing entry's shape)
3. Run: python generate_go_pages.py   (no CLI flags — it's a static data
   list, not a per-tool generator; the script regenerates go/[slug]/index.html
   for every entry currently in TOOLS)
4. Check the generated page for any page-specific inline <script> block against
   this site's CSP (`_headers` at repo root) — RankerNest hit a real bug where
   its CSP silently blocked every /go/ page's inline redirect script for days
   because it lacked 'unsafe-inline'; verify the same isn't happening here
   before assuming the redirect actually fires.
5. Hand the finished go/[slug]/ path to rankertoolai-qa / rankertoolai-deploy
```
...
### Mode D: Go Bridge Page Request

```
Input: { "tool": "Jasper", "request": "go_page" }
```
